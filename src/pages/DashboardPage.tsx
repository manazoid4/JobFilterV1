...
const FREQ_OPTIONS = [
  ...
  { value: 'instant', label: 'DAILY PRIORITY CHECK' },
];
          ...
          <div className="grid gap-2">
            {activeAlerts.map(a => (
              <div key={a.id} className={`flex flex-wrap items-center justify-between gap-2 border-2 border-[var(--line)] p-2 text-xs font-black uppercase ${a.active ? 'bg-[var(--bg-main)]' : 'bg-white text-[var(--muted)]'}`}>
                <span>
                  {a.active ? 'Active' : 'Paused'} · {TRADES.find(t => t.value === a.trade)?.label ?? a.trade} · {a.postcode_outward} · {a.radius_miles ?? 25}mi · {a.frequency === 'instant' ? 'Priority daily' : a.frequency}
                </span>
