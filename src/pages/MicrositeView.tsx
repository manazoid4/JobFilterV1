...
import { type MicrositeProfile } from '../lib/microsite';

// Pure presentational firm page. Used by the DB-backed server route
// (app/[slug]) and the URL-param client fallback (app/pro/[slug]).
export function MicrositeView({ slug, profile }: { slug: string; profile: MicrositeProfile }) {
  const firm = profile;
  ...
  return (
    <main className="page-shell grid gap-5 py-8 pb-24 md:pb-8">
      <section className="jf-box bg-[var(--navy)] p-7 text-white">
        ...
        {firm.areas ? <p className="mt-4 text-lg font-black text-white/90">Covering {firm.areas}</p> : null}
        {firm.years ? (
