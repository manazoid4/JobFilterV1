  ...
  return (
    <main className="page-shell grid gap-5 py-8 pb-24 md:pb-8">
      <section>
        ...
        <h1 className="headline mt-2 text-4xl leading-none sm:text-5xl">NEEDS ACTION</h1>
      </section>
      ...
      {needsAction.length === 0 ? (
        <section className="jf-box bg-white p-6">
          <h2 className="headline text-2xl sm:text-3xl">NO NEW LEADS</h2>
...
function OutcomeStat({ label, value }: { label: string; value: number }) {
  return (
    <div>
      ...
      <p className="headline mt-1 text-3xl sm:text-4xl">{value}</p>
    </div>
  );
}
