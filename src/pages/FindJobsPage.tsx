  ...
  return (
    <main className="page-shell grid gap-5 py-8 pb-24 md:pb-8">
      ...
      <section className="jf-box bg-white p-7">
        <p className="micro-label text-[var(--orange)]">LIVE SCANNER — 3 FREE SCANS — NO CREDIT CARD REQUIRED</p>
        <h1 className="headline mt-2 text-3xl leading-none sm:text-4xl">FIND PUBLIC WORK WORTH BIDDING ON</h1>
              ...
              {!DEV_MODE && !unlimitedTester && displayedLeads.length > 0 && (
                <section className="jf-box bg-[var(--yellow)] p-5">
                  ...
                  <h2 className="headline mt-2 text-3xl leading-none sm:text-4xl">
                    {goldCount > 0
                      ? `${goldCount} GOLD LEAD${goldCount !== 1 ? 'S' : ''} NEAR ${result?.outward || postcode.trim().split(' ')[0].toUpperCase()} — WHO&apos;S THE BUYER? WHAT&apos;S THE DEADLINE?`
                      : 'THESE LEADS HAVE BUYERS. SEE WHO TO CONTACT BEFORE ANOTHER CONTRACTOR DOES.'}
                  </h2>
                  <p className="mt-2 text-sm font-black text-[var(--ink)]/80">
                    Full access shows: buyer name, published value where available, submission deadline, and the official response route. One job won at £2,000+ covers 4 years at £39/mo.
                  </p>
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    <Link href="/pricing" className="jf-button bg-[var(--ink)] text-white">UNLOCK BUYER DETAILS — £39/MO →</Link>
                    <span className="text-xs font-black text-[var(--ink)]/60">No credit card required to scan · public tender · not exclusive</span>
                  </div>
                </section>
              )}
      ...
      {!hasScanned && !loading && !fillWeekLoading && (
        <section className="jf-box bg-[var(--navy)] p-6 text-center text-white" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Ccircle cx=\'30\' cy=\'30\' r=\'1.5\' fill=\'%23E3B72A\' opacity=\'0.2\'/%3E%3C/svg%3E")' }}>
          ...
          <h2 className="headline mt-3 text-3xl leading-none sm:text-5xl">FIND OUT IF THERE&apos;S PUBLIC WORK NEAR YOU RIGHT NOW.</h2>
          <p className="mt-3 font-black text-white/70">
            Enter your postcode. Pick your trade. 10 seconds to see what&apos;s live in your patch today.
          </p>
