    ...
    <main className="page-shell grid gap-6 py-8 pb-16 text-[var(--ink)]">
      ...
      <section className="ops-panel bg-[var(--ink)] p-7 text-white">
        ...
        <h1 className="headline mt-3 max-w-4xl text-5xl leading-none text-white md:text-7xl">
          STOP WASTING BID TIME ON TENDERS THAT DON'T FIT YOUR FIRM.
        </h1>
        <p className="mt-5 max-w-2xl text-xl font-bold text-white/85">
          Public tenders come at you fast. JobFilter reads current Find a Tender notices and tells you whether to BID, WATCH, pursue a SUBCONTRACT route, or SKIP — before you burn days on a wrong fit.
        </p>
        ...
        <p className="mt-4 text-sm font-black text-[var(--yellow)]/80">Find a Tender is free and public. The £39/mo pays for firm-aware qualification, evidence and workflow — not privileged access.</p>
        <p className="mt-1 text-sm font-black text-white/60">No credit card required for the free coverage check. Scan first, pay only if the coverage fits.</p>
      </section>
      ...
      <section className="ops-panel bg-[var(--yellow)] p-7 text-[var(--ink)]">
        <p className="micro-label text-[var(--ink)]">SCAN FIRST. PAY ONLY IF IT FITS.</p>
        <h2 className="headline mt-3 text-4xl leading-none md:text-6xl">CHECK YOUR PATCH BEFORE YOU COMMIT.</h2>
        <p className="mt-3 max-w-2xl font-black text-[var(--ink)]/75">Run a free scan against the current Find a Tender feed. If leads are there for your trade and region, activate. If not, you've lost nothing.</p>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        ...
        <p className="mt-4 text-sm font-black text-[var(--ink)]/70">Official tenders are public. You pay for qualification, evidence and workflow — not for access to a notice.</p>
      </section>
    </main>
  );
}
