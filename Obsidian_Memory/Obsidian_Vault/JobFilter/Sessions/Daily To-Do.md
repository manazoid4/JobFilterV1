...
- [x] Fix scan-limit expired message — name specific upgrade value (buyer name, value, deadline, how to bid)
...
- [ ] **Verify wins API in production** — Check `WinStatsBanner` renders when real `lead_outcomes` rows exist with `status='won'`. Test with a real postcode.
...
| Won leaderboard (WinStatsBanner + /api/wins/stats) | ✅ BUILT (2026-07-29) |
