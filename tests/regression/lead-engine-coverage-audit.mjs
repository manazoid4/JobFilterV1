...
const outDir = path.resolve('tests/.artifacts');
await fs.mkdir(outDir, { recursive: true });
await fs.writeFile(path.join(outDir, 'lead-engine-coverage-audit.json'), JSON.stringify(report, null, 2));
await fs.writeFile(path.join(outDir, 'lead-engine-coverage-audit.md'), [
  '# JobFilter Lead Engine Coverage Audit',
  '',
